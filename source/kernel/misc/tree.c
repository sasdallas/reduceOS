...
void tree_node_destroy(tree_node_t* node) {
    foreach (child, node->children) { tree_node_destroy((tree_node_t*)child->value); }
...
void tree_node_free(tree_node_t* node) {
    if (!node) return;
    foreach (child, node->children) { tree_node_free(child->value); }

    kfree(node);
}

// tree_free(tree_t *tree) - Frees all of the nodes in a tree, but not their contents
void tree_free(tree_t* tree) { tree_node_free(tree->root); }
...
size_t tree_count_children(tree_node_t* node) {
    ...
    foreach (child, node->children) { out += tree_count_children((tree_node_t*)child->value); }
    return out;
}
...
void tree_remove(tree_t* tree, tree_node_t* node) {
    ...
    tree->nodes--;
    list_delete(parent->children, list_find(parent->children, node));
    foreach (child, node->children) { ((tree_node_t*)child->value)->parent = parent; }

    list_merge(parent->children, node->children);
    kfree(node);
}

// tree_remove_reparent_root(tree_t *tree, tree_node_t *node) - Remove this node and move its children into the root children
