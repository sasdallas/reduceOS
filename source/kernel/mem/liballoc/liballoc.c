...
static void liballoc_dump()
{
	...
	serialPrintf( "liballoc: ------ Memory data ---------------\n");
	serialPrintf( "liballoc: System memory allocated: %i bytes\n", l_allocated );
	serialPrintf( "liballoc: Memory in used (malloc'ed): %i bytes\n", l_inuse );
	serialPrintf( "liballoc: Warning count: %i\n", l_warningCount );
	serialPrintf( "liballoc: Error count: %i\n", l_errorCount );
	serialPrintf( "liballoc: Possible overruns: %i\n", l_possibleOverruns );

#ifdef DEBUG
		while ( maj != NULL )
		{
			serialPrintf( "liballoc: %x: total = %i, used = %i\n",
						maj, 
						maj->size,
						maj->usage );
			...
			while ( min != NULL )
			{
				serialPrintf( "liballoc:    %x: %i bytes\n",
							min, 
							min->size );
				min = min->next;
			}
...
static struct liballoc_major *allocate_new_page( unsigned int size )
{
	...
	struct liballoc_major *maj;
		...
		if ( maj == NULL ) 
		{
			l_warningCount += 1;
			#if defined DEBUG || defined INFO
			serialPrintf( "liballoc: WARNING: liballoc_alloc( %i ) return NULL\n", st );
			FLUSH();
			#endif
...
void *PREFIX(malloc)(size_t req_size)
{
	...
	if ( l_memRoot == NULL )
	{
		#if defined DEBUG || defined INFO
		#ifdef DEBUG
		serialPrintf( "liballoc: initialization of liballoc " VERSION "\n" );
		#endif
		...
		if ( l_memRoot == NULL )
		{
		  liballoc_unlock();
		  #ifdef DEBUG
		  serialPrintf( "liballoc: initial l_memRoot initialization failed\n", p); 
		  FLUSH();
		  #endif
		  return NULL;
		}
		...
		#ifdef DEBUG
		serialPrintf( "liballoc: set up first memory major %x\n", l_memRoot );
		FLUSH();
		#endif
	}
	...
	while ( maj != NULL )
	{
		...
		{
			#ifdef DEBUG
			serialPrintf( "CASE 1: Insufficient space in block %x\n", maj);
			FLUSH();
			#endif
		...
		if ( maj->first == NULL )
		{
			...
			#ifdef DEBUG
			serialPrintf( "CASE 2: returning %x\n", p); 
			FLUSH();
			#endif
		...
		if ( diff >= (size + sizeof(struct liballoc_minor)) )
		{
			...
			#ifdef DEBUG
			serialPrintf( "CASE 3: returning %x\n", p); 
			FLUSH();
			#endif
			liballoc_unlock();		// release the lock
			return p;
		...
		while ( min != NULL )
		{
				...
				if ( min->next == NULL )
				{
					...
					if ( diff >= (size + sizeof( struct liballoc_minor )) )
					{
						...
						#ifdef DEBUG
						serialPrintf( "CASE 4.1: returning %x\n", p); 
						FLUSH();
						#endif
						liballoc_unlock();		// release the lock
						return p;
				...
				if ( min->next != NULL )
				{
					...
					if ( diff >= (size + sizeof( struct liballoc_minor )) )
					{
						...
						#ifdef DEBUG
						serialPrintf( "CASE 4.2: returning %x\n", p); 
						FLUSH();
						#endif
		...
		{
			#ifdef DEBUG
			serialPrintf( "CASE 5: block full\n");
			FLUSH();
			#endif
	...
	#ifdef DEBUG
	serialPrintf( "All cases exhausted. No memory available.\n");
	FLUSH();
	#endif
	#if defined DEBUG || defined INFO
	serialPrintf( "liballoc: WARNING: PREFIX(malloc)( %i ) returning NULL.\n", size);
	liballoc_dump();
	FLUSH();
	#endif
	return NULL;
...
void PREFIX(free)(void *ptr)
{
	...
	if ( min->magic != LIBALLOC_MAGIC ) 
	{
		   ...
		   )
		{
			...
			#if defined DEBUG || defined INFO
			serialPrintf( "liballoc: ERROR: Possible 1-3 byte overrun for magic %x != %x\n",
								min->magic,
								LIBALLOC_MAGIC );
			FLUSH();
			#endif
	...
	#ifdef DEBUG
	serialPrintf( "liballoc: %x PREFIX(free)( %x ): ", 
