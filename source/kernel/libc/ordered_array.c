...
#include "include/libc/ordered_array.h" // Main header file
...
void insertOrderedArray(type_t item, ordered_array_t *array) {
    ASSERT(array->less_than, "insertOrderedArray()", "invalid array->less_than"); // Assert the less_than (check if it's valid)
...
type_t lookupOrderedArray(uint32_t i, ordered_array_t *array) {
    ASSERT(i < array->size, "lookupOrderedArray()", "item index is greater than array size"); // Assert i (check if it's valid)
