...
static uint32_t contract(uint32_t newSize, heap_t* heap) {
    ...
    ASSERT(newSize < heap->endAddress - heap->startAddress, "contract()", "new size is greater than current size");
...
heap_t* createHeap(uint32_t startAddress, uint32_t endAddress, uint32_t maxAddress, uint8_t supervisor, uint8_t readonly) {
    ...
    ASSERT(startAddress % PAGE_ALIGN == 0, "createHeap", "startAddress is not page aligned");
    ASSERT(endAddress % PAGE_ALIGN == 0, "createHeap", "endAddress is not page aligned");
...
void free(void* p, heap_t* heap) {
    ...
    ASSERT(header->magic == HEAP_MAGIC, "free()", "header has invalid magic number");
    ASSERT(footer->magic == HEAP_MAGIC, "free()", "footer has invalid magic number");
    ...
    if (testHeader->magic == HEAP_MAGIC && testHeader->isHole) {
        ...
        ASSERT(iterator < heap->index.size, "free()", "unable to find header to remove");
        // Remove it.
        removeOrderedArray(iterator, &heap->index);
    }
