...
int ide_tests() {
    if (1) {
        ...
        ideReadSectors(drive, 1, 2, (uint32_t)sector);
        ...
        ideWriteSectors(drive, 1, 2, (uint32_t)write_buffer);

        // Now, verify (reuse write_buffer to save memory)
        memset(write_buffer, 0x0, 512);
        ideReadSectors(drive, 1, 2, (uint32_t)write_buffer);
        ...
        ideWriteSectors(drive, 1, 2, (uint32_t)sector);
        printf("DONE\n");
        ...
        ideReadSectors(drive, 4, 0, (uint32_t)comparison_buffer);
        ...
        printf("\tRestoring sector...");
        ideWriteSectors(drive, 4, 0, (uint32_t)comparison_buffer);
        printf("DONE\n");
        ...
        ideReadSectors(drive, 1, lba, (uint32_t)buffer);
    ...
    if (1) {
        ...
        if (ret->flags != VFS_FILE) {
            ...
            if (fatReadInternal(ret, buffer, ret->length) == EOF) {
                for (uint32_t j = 0; j < ret->length; j++) serialPrintf("%c", buffer[j]);
                printf("PASS\n");
            } else {
...
int ext2_tests() {
    ...
    if (!test_file) {
        ...
        test_file->write(test_file, 0, strlen(a), (uint8_t*)a);
        kfree(a);
    ...
    uint32_t ret = test_file->read(test_file, 0, test_file->length, comparisonBuffer);
    ...
    if (ret == test_file->length) {
        ...
        strcpy((char*)buffer, "reduceOS is cool");
        ...
        bool checkFail = false;
        for (unsigned int i = 0; i < test_file->length; i++) {
