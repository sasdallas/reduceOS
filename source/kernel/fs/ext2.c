...
int allocate_block(ext2_t *fs) {
    ...
    if (!blockNumber) {
        panic("ext2", "ext2_allocateBlock", "No available blocks");
    }
    ...
    if (startBlock == endBlock) {
        ...
        for (blockOffset = startBlock; blockOffset < endBlock; blockOffset++, blocksRead++) {
            if (blockOffset == startBlock) {
                ...
                memcpy((uint8_t*)(((uintptr_t)buf) + ((uintptr_t)offset % fs->block_size)), buffer, fs->block_size - (offset % fs->block_size));
...
static int ext2_createEntry(fsNode_t *parent, char *name, uint32_t inode) {
    ...
    serialPrintf("\tinode = %i\n\tentrySize = %i\n\tname length = %i\n\tfile type = %i\n\tname = %s\n", inode, entrySize, strlen(name), 0, name);
...
int ext2_read(fsNode_t *node, off_t offset, uint32_t size, uint8_t *buffer) {
    ...
    if (startBlock == endBlock) {
        ...
        ext2_readInodeBlock(fs, inode, startBlock, buf);
        memcpy(buffer, (uint8_t*)(((uintptr_t)buf) + ((uintptr_t)offset % fs->block_size)), sizeToRead); // Does a similar thing in the FAT driver
    } else {
        uint32_t blockOffset;
        uint32_t blocksRead = 0;
