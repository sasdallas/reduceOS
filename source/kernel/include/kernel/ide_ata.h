...
#include <libk_reduced/sleep.h> // Sleep function
...
uint8_t ideAccessATA(uint8_t direction, uint8_t drive, uint64_t lba, uint8_t sectorNum, uint32_t edi); // Access an ATA drive (direction specifies what operation to perform)
uint8_t ideReadATAPI(uint8_t drive, uint32_t lba, uint8_t sectorNum, uint32_t edi); // Read from an ATAPI drive.
int ideReadSectors(uint8_t drive, uint8_t sectorNum, uint64_t lba, uint32_t edi); // Read from an ATA/ATAPI drive.
